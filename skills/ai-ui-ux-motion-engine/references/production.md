# Production after creative agreement

Read only after the entrypoint's PRODUCTION condition is met, or for a bounded
audit/fix that needs these checks. Command paths are relative to the skill root.
Speed targets, provider routing and private proofs apply after agreement.
Creative delegation alone is not a detailed brief: return to BRIEFING to develop
and discuss one before starting this workflow.
...
6. Create a cinematic brief from
   ...
   Read [production-contract.md](production-contract.md) fully.
...
[tool-connections.md](tool-connections.md) completely before
provider work.

For an undeveloped idea, return to BRIEFING in the entrypoint.
...
After creative agreement, select the professional route and bounded proof efficiently. It cannot guarantee that a stochastic provider’s first render
will pass.
...
Read [cinematic-intake.md](cinematic-intake.md),
[generated-product-scrubber.md](generated-product-scrubber.md) and
[cinematic-prompts.md](cinematic-prompts.md) completely when this
gate activates.
...
Read [media-pipeline.md](media-pipeline.md).
...
- [creative-briefing.md](creative-briefing.md): conversational discovery,
  evolving plain-language brief and agreement before production for any new idea.
- [production-contract.md](production-contract.md): fresh-task
  contract, evidence schema, route/model selection and pre-spend validation.
- [cinematic-intake.md](cinematic-intake.md): mandatory questions,
  pushback, tiers, spend and stop rules.
- [cinematic-prompts.md](cinematic-prompts.md): exact reusable
  single-action, flagship and illustrative prompt contracts.
- [generated-product-scrubber.md](generated-product-scrubber.md):
  cinematic production and scroll-delivery golden path.
- [media-pipeline.md](media-pipeline.md): references, ffmpeg,
  all-intra video, frames, posters and delivery.
- [tool-connections.md](tool-connections.md): provider preflight and
  external-tool boundaries.
- [motion-patterns.md](motion-patterns.md): code-native motion.
- [framework-recipes.md](framework-recipes.md): stack-specific
  implementation.
- [accessibility-performance.md](accessibility-performance.md):
  motion safety and performance.
- [prompts.md](prompts.md): user-facing request templates.
- [source-coverage.md](source-coverage.md): provenance and known
  limitations.
